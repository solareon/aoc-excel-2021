...
# Day 15
Find a path through a grid. Much pasting and lots of LAMBDA() and arrays yields a result. Part 2 goes from 100x100 to 500x500 pushing the limits of Excel and the memory in my desktop. The resultant files are too large to post but I will paste the formula here that does the heavy lifting. =LET(x,WORKINGTABLEBOTTOMRIGHT,y,LOOKUPTABLEBOTTOMRIGHT,IFS(OFFSET(x,0,1)<(x-OFFSET(y,0,1)),y+OFFSET(x,0,1),OFFSET(x,1,0)<(x-OFFSET(y,1,0)),y+OFFSET(x,1,0),OFFSET(x,0,-1)<(x-OFFSET(y,0,-1)),y+OFFSET(x,0,-1),OFFSET(x,-1,0)<(x-OFFSET(y,-1,0)),y+OFFSET(x,-1,0),TRUE,x))
